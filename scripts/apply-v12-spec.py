...
app.write_text(r'''document.addEventListener('DOMContentLoaded', () => {
  ...
  const defaultRange = () => { const end = today(); const start = new Date(); start.setDate(start.getDate() - 29); return { from: start.toISOString().slice(0, 10), to: end }; };
  ...
  document.getElementById('profile-save')?.addEventListener('click', () => { const old = current(); const p = profiles.save({ firebaseUid: old.firebaseUid, email: document.getElementById('profile-email').value.trim(), displayName: old.displayName || document.getElementById('profile-name').value.trim(), photoURL: old.photoURL || '', customPhoto: old.customPhoto || old.photo || '', fullName: document.getElementById('profile-name').value.trim(), hub: document.getElementById('profile-hub').value.trim(), position: document.getElementById('profile-position').value, employeeId: document.getElementById('profile-employee').value.trim(), driverId: document.getElementById('profile-driver').value.trim() }, editingProfileId); editingProfileId = p.profileId; renderProfile(); alert('บันทึกโปรไฟล์แล้ว'); });
  ...
  document.getElementById('profile-share')?.addEventListener('click', () => alert('Share App จะเปิดใช้งานหลังสร้าง GitHub Release V1.2 และตรวจสอบ APK URL จริงแล้ว'));
  document.getElementById('profile-support')?.addEventListener('click', () => showView('view-support')); document.getElementById('support-back')?.addEventListener('click', () => showView('view-settings'));
